...
def deployd(portal_actual):	
	deployds_posibles = 8 - portal_actual.deployds

	#if deployds_posibles > 0 and portal_actual.team  != 'RES':
	if deployds_posibles > 0 and portal_actual.team  != 'ENL':
		print('deploids = ' + str(deployds_posibles) )
		...
		gris = check_output_timeout('ImageMagick-6.8.9-0\\convert -crop 1x1+790+456 {}\\screen{}.png -unique-colors txt:'.format(dirdocs,'gl').split(' ') , universal_newlines=True)
		if '#282828' in gris:touch_x_y(16000, 17000) #disable mock
		print(gris)

		check_output_timeout('adb shell screencap -p /sdcard/bstfolder/Documents/img_temp/screen{}.png'.format('gl').split(' ') )
		print('ImageMagick-6.8.9-0\\convert {}\\screen{}.png -crop 1x1+1087+715 -threshold 50% txt:'.format(dirdocs,'gl'))
		...
		if 'white' in str(candeployd) and 'black' in str(ops):
			print('ok')
			touch_x_y(18415, 25900)#deployd
			sleep(2)
			for i in range(deployds_posibles): #Cantidad de deploids posibles (siendo 8 maximo)
				touch_x_y(2750, 9425)#Update
				sleep(10)
			touch_x_y(2820, 23920)#Done
			touch_x_y(2820, 23920)#Done
			return(1)
			sql.execute('update portals set deployds = deployds -1 where lat = {} and lon = {} '.format( 8  ,portal_actual.lat ,portal_actual.lon) )
		return(0)
	else:
		return(1)


def hackear( portal_actual,distancia_actual,opcion = 1): #opcion 1 
	print('hackear')
	print( 'Hakeando {} LV{} {} '.format(portal_actual.nombre,portal_actual.lvl , distancia_actual))
	...
	if opcion == 2:
		...
		for nombre,sec in final:
			...
			touch_x_y(  up = 1 )
		sleep(6)#despues lyphos de esperar
...
def portal_siguiente_class(port_act,date_now = datetime.now(),Hack_time = Hack_time - timedelta(seconds=10) ,Hack_time_burn = Hack_time_burn,min_lvl = 0,solo_print=0,team_distinct='',max_deployds=8): #team=ENL|RES|NEU
	print('portal_siguiente_class  deploids=' + str(max_deployds))
	distancia_menor = 9999
	portal_ret=0

	extras = ''
	if team_distinct != '': extras = " where team <> '{}'  and deployds <= '{}' ".format(team_distinct ,max_deployds)
...
def select_portal(lat,lon):
	...
	if row is  None: 
		...
		if row1 is  None: 
			...
			var.lvl = row1['lvl']
			var.team = row1['team']
			var.deployds = row1['deployds']
			var.ultimo_hack = row1['ultimo_hack']
			var.hacks_restantes = row1['hacks_restantes']
...
def gplyph(portal_actual):
	...
	if 'white' in str(hack) and 'black' in str(ops):
		...
		touch_x_y(22150, 26560,tiempo = 3)#hack
...
def unload_capsule(var):
	...
	for i in range(var)	:
		touch_x_y(26641, 21069,tiempo=1)
		touch_x_y(2130, 18682,tiempo=1)
