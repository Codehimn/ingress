...
;~ 	Do
	For $Y = 0 To UBound($Portales) - 1
		...
		If $Portales[$Y][3] >= 1 And (Abs(@MIN - $Portales[$Y][4]) > 5 And Abs(@MIN - $Portales[$Y][4]) < 55) Then ;si quedan hacks disponibles, y si lleva mas de 5 min
			...
			EndIf
		EndIf
		ConsoleWrite($Y  & ","&($Portales[$Y][3] >= 1) &"," &(Abs(@MIN - $Portales[$Y][4]) > 5 And Abs(@MIN - $Portales[$Y][4]) < 55)  &"," & ($Portales[$Y][3] >= 1 And (Abs(@MIN - $Portales[$Y][4]) > 5 And Abs(@MIN - $Portales[$Y][4]) < 55) )& "  " & Abs(@MIN - $Portales[$Y][4]) & "  " & $Portales[$Y][4] & @CRLF)
	Next
