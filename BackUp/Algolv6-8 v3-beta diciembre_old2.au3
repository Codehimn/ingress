...
;~ RunWait("am start -a com.nianticproject.ingress -n com.nianticproject.ingress/com.nianticproject.ingress.NemesisActivity" )
	StdinWrite("am startservice --ei lat " & $lat & " --ei long " & $long & " -n com.lexa.fakegpsdonate/.FakeGPSService")
	StdinWrite("am force-stop com.lexa.fakegpsdonate")

	Sleep($DistanciaMS)
	WinActivate("BlueStacks")
	Sleep(2000)
;~ 	MouseClick("left",1170, 480,1,0)
...
StdinWrite($pid, 'sqlite3 /data/data/com.android.providers.settings/databases/settings.db "' & "update secure set value=0 where name='mock_location' " & '" ;' & @CRLF)
	...
	For $i = 1 To 15
;~ 		MouseClick("left",1337, 273,1,0)
		MouseClick("left", 1877, 373, 1, 0)

		Sleep(100)
	Next
	RunWait("adb shell am force-stop com.lexa.fakegpsdonate")
	Sleep(1000)
